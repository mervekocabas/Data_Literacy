...
We also analyse the IEEE citations in both corporate co-authored papers and academia papers by performing a one sided Mann-Whitney U test. This test does not assume that the data is normally distributed, making it a useful test as the IEEE citations are not normally distributed in our dataset. 

The Mann-Whitney U test [3] compares two independent samples by ranking all observations together and summing the ranks for each group. The U statistic [3] measures how often a value in one group is greater than a value in the other is calculated as follows,
...
where $n_x$ and $n_y$ are the sample sizes for both the groups and $R_1$ and $R_2$ are the rank sums for both the groups respectively. 

Under the null hypothesis, the distribution of \( U \) is approximately normal with mean $\mu_U$ [3] and standard deviation $\sigma_U$ [3] as follows,
...
The z-score $Z$ is compared to a critical value $\alpha$ to determine statistical significance, leading to either acceptance or rejection of the null hypothesis [3].
...
[3] H. B. Mann and D. R. Whitney, “On a Test of Whether one of Two Random Variables is Stochastically Larger than the Other,” The Annals of Mathematical Statistics, vol. 18, no. 1, pp. 50–60, 1947, doi: 10.1214/aoms/1177730491.
\end{document}
